...
function renderLancamentosJazz(){

    const ultimosAlbums = albumsReg.filter(album => album.genero.includes("JAZZ")).splice(-12);
   ...
   `;
        ultimosJazz.prepend(novoCard);
        }))
    };


function renderLancamentosHipHop() {

    const ultimosAlbums = albumsReg.filter(album => album.genero.includes(("HIP-HOP") || ("HIPHOP"))).splice(-12);
   ...
   `;
        ultimosHipHop.prepend(novoCard);
        }))
    };

function renderLancamentosAlternativa() {

    const ultimosAlbums = albumsReg.filter(album => album.genero.includes("ALTERNATIV")).splice(-12);
...
function renderLancamentosIndie() {

    const ultimosAlbums = albumsReg.filter(album => album.genero.includes("INDIE")).splice(-12);
   ...
   `;
        ultimosIndie.prepend(novoCard);
        }))
    };



function renderLancamentosRB() {

    const ultimosAlbums = albumsReg.filter(album => album.genero.includes("R&B")).splice(-12);

    ultimosAlbums.forEach((album, index) => {
        ...
        <article>
            <h2>${nome}</h2>
            <h3>${album.artista}<h3>
            <p data-index="${index}" onclick="addToCart(${index})"><span class="material-symbols-outlined" style="font-size: 16px">
            shopping_cart_checkout
            </span> R$${album.valor.toFixed(2)}</p>
        </article>
   `;
        ultimosRB.prepend(novoCard);
        })
    };

function renderLancamentosPop() {

    const ultimosAlbums = albumsReg.filter(album => album.genero.includes("POP")).splice(-12);
   ...
   `;
        ultimosPop.prepend(novoCard);
        }));
}

function renderLancamentosRock() {

    const ultimosAlbums = albumsReg.filter(album => album.genero.includes("ROCK")).splice(-12);
...
renderLancamentosRock();
renderLancamentosPop();
renderLancamentosRB();
renderLancamentosIndie();
renderLancamentosAlternativa();
renderLancamentosJazz();
renderLancamentosHipHop();
updateSliderPop();
updateSliderRock();
updateSliderRB();
updateSliderIndie();
